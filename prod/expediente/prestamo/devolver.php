			     ...
			     if($rs && !$rs->EOF)
			     {
				     /****************************/
				      ...
				      while($rs && !$rs->EOF) 
				      {
					    ...
					    $tblPrestados.=" <tr  align='center'><td class='titulos3' colspan='11' align='center'><input type='submit' class='botones' value='Devolver' onClick='return devolver();'></td></tr></table><br>";
			     }
				 else
				 {
					 $tblPrestados="<center><table width='60%' border=0 cellpadding=0 cellspacing=2 class='borde_tab'><tr><td class='listado5' colspan='2'><center><font color=red size=3>No existen Expedientes Prestados con el criterio de B&uacute;squeda</font></center></td></tr></table></center>";
				 }
				break;
			case 'combo':
...
function devolver()
{
	if(validar())
	{
		window.document.frmSolPrest.action = './formEnvio.php?tipoEnvio=devolver';
		window.document.frmSolPrest.submit();
	}
	else return false
}
...
<table width="60%" align="center" border=0 cellpadding=0 cellspacing=2 class='borde_tab'>
		<tbody>
		<tr>
			<td  class="titulos4" colspan="2">B&Uacute;SQUEDA: EXPEDIENTES PARA DEVOLVER</td>
		</tr>
		<tr>
