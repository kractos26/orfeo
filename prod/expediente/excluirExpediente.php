...
if( isset( $_POST['expSeleccionados'] ) && $_POST['expSeleccionados'] != "" )
{
    ...
    foreach( $arrExpSeleccionados as $clave => $numExpediente )
    {
        ...
        {
            $mensaje = "Va a excluir &eacute;ste documento del(os) Expediente(s) seleccionado(s). <br> Est&aacute; seguro?";
        }
    }
    ...
    if( isset( $_POST['confirmaIncluirExp'] ) && $_POST['confirmaIncluirExp'] == "EXCLUIR_EXP" )
    {
        foreach( $arrExpSeleccionados as $clave => $numExpediente )
        {
            ...
            {
                print '<hr><font color=red>No se excluy&oacute; este radicado del expediente No. '.$numExpediente.'. Por favor intente de nuevo.</font><hr>';
                break;
            }
        }
        ?>
