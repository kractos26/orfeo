...
if ($SecSuperAux->UsrPerm != 0) {
    if ($SecSuperAux->SecureCheck($verrad) == true) {
        $SecSuper=true;
    }
}


if ((( $verSeguridadExp == 0 || $verSeguridadExp == 2 ) && $codusuario == $verCodusuario && $dependencia == $verDependencia)||$SecSuper==true) {
    $verradPermisos = "Full";
} elseif (($verSeguridadExp == 1 && $codusuario == $verCodusuario && $dependencia == $verDependencia && $_SESSION["codusuario"] == 1)||$SecSuper==true) {
    $verradPermisos = "Full";
} else {
    $verradPermisos = "Otro";
    $mostrar_opc_envio = 0;
