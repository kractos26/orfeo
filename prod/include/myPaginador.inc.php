...
class myPaginador{
	...
	{
		$salida=(strtoupper($this->tipoOrden)=="ASC" || is_null($this->tipoOrden))?$this->imgOrdASC:$this->imOrdDESC;
		return $salida;
	}
	...
	function generarPagina($columnasTitulo,$claseTitulo="titulos3",$noHayResultados='NO se Encontraron Resultados'){	
		...
		while(!$resultado->EOF){
			...
			if($this->modoPintarCeldas){
				...
				echo "\t <tr {$claseTr}> \n \t \t ";
				for($n=0; $n< $numCols;$n++)
				{	if (!function_exists($this->funcionPintaFilas))  die ("No existe function ".$this->funcionPintaFilas);
