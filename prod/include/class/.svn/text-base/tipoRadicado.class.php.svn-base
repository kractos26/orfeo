...
class TipRads {
    ...
    function SetInsDatosTipRad($datos) {
        ...
        $sql = "select DEPE_RAD_TP$datos[SGD_TRAD_CODIGO] from DEPENDENCIA ";
        $ok2 = $this->cnn->conn->Execute($sql);
        if ($ok2 && !$ok2->EOF) {
            $ok2 = true;
        } else {
        ...
        $sql = "select SGD_TPR_TP$datos[SGD_TRAD_CODIGO] from SGD_TPR_TPDCUMENTO ";
        $ok3 = $this->cnn->conn->Execute($sql);
        if ($ok3 && !$ok3->EOF) {
            $ok3 = true;
        } else {
        ...
        $sql = "select USUA_PRAD_TP$datos[SGD_TRAD_CODIGO] from USUARIO ";
        $ok4 = $this->cnn->conn->Execute($sql);
        if ($ok4 && !$ok4->EOF) {
            $ok4 = true;
        } else {
        ...
        $sql = "select SGD_TPR_TP$datos[SGD_TRAD_CODIGO] from SGD_TIP3_TIPOTERCERO ";
        $ok6 = $this->cnn->conn->Execute($sql);
        if ($ok6 && !$ok6->EOF) {
            $ok6 = true;
        } else {
    ...
    function SetDelDatosTipRad($dato) {
        $this->cnn->conn->BeginTrans();
        $sql = "SELECT count(RADI_NUME_RADI) as CNT FROM RADICADO WHERE cast(" . $this->cnn->conn->substr . "(cast (RADI_NUME_RADI as VARCHAR),14,1)AS integer) =" . $dato;
