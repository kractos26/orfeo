 ...
 * @ Updates 1.1  Agregado metodo Get_Descripcion  HLP.
 */
class Resoluciones
{
...
/**
 * Retorna la resolucion dado el codigo.
 *
 * @param integer $idRes
 * @return String  Nombre de la Resolucion, false si no existe.
 */
function Get_Descripcion($idRes)
{
    $ret = false;
    $sql = "SELECT SGD_TRES_DESCRIP AS DESCRIP FROM SGD_TRES_TPRESOLUCION WHERE SGD_TRES_CODIGO=$idRes";
    global $ADODB_COUNTRECS;
    $ADODB_COUNTRECS = true;
	$rs = $this->cnn->Execute($sql);
    $ADODB_COUNTRECS = false;
	if ($rs->RecordCount() > 0)   $ret = $rs->fields['DESCRIP'];
    $rs->Close();
    return $ret;
}
}
