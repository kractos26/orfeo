<?
	...
	case 'oci805':
		...
		$query = "select  
			...
			$fecha_mes = substr($fecha_ini,0,7);
					...
					and a.sgd_deve_codigo=b.sgd_deve_codigo
					and a.sgd_fenv_codigo=d.sgd_fenv_codigo
					and '.$db->conn->substr.'(cast (a.radi_nume_sal AS varchar2(14)), 5, 3)=c.depe_codi
					and a.sgd_deve_codigo is not null';
	break;		
	...
	default:
			...
			$where_isql = ' WHERE a.sgd_deve_fech BETWEEN
					...
					and a.sgd_deve_codigo=b.sgd_deve_codigo
					and a.sgd_fenv_codigo=d.sgd_fenv_codigo
					and cast( '.$db->conn->substr.'(cast (a.radi_nume_sal AS VARCHAR), 5, 3) as integer)=c.depe_codi
					and a.sgd_deve_codigo is not null
			';
	}
?>
