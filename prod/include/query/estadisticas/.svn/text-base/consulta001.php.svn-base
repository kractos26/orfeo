...
$sWhereFec = "" . $db->conn->SQLDate('Y/m/d H:i:s', 'R.RADI_FECH_RADI') . " between  '$desde'
				and  '$hasta'";
switch($db->driver)
{
	case 'mssql':
	...
	{	if($tipoDocumento=='9999')
		...
		$queryEDetalle = "SELECT $radi_nume_radi as RADICADO
                    ...
                    ,$tmp_substr(cast (dir.sgd_dir_tipo as VARCHAR(4)),2,2) as COPIA
        ...
        case 'oci8po':
	{
		...
		$queryEDetalle = "SELECT $radi_nume_radi as RADICADO
					...
					,r.RADI_PATH as HID_RADI_PATH {$seguridad}
                                        ,d.depe_nomb as DEP_ASIG
					, dir.SGD_DIR_NOMREMDES as REMITENTE
                                        ,$tmp_substr(cast(dir.sgd_dir_tipo as VARCHAR(4)),3,3) as COPIA
	  		...
	  		WHERE   $sWhereFec $whereTipoRadicado $resolucioni AND $tmp_substr(cast(dir.sgd_dir_tipo as VARCHAR(4)),3,3) IS NULL ";
				$orderE = "	ORDER BY $orno $ascdesc";
			 /** CONSULTA PARA VER TODOS LOS DETALLES 
			 */ 
