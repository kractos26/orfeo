...
switch($db->driver)
	...
	$queryE = "SELECT b.USUA_NOMB as NOMBRE, COUNT(r.RADI_NUME_RADI) as TOTAL_MODIFICADOS,b.USUA_DOC as HID_COD_USUARIO
				  ...
				  WHERE b.USUA_DOC = h.USUA_DOC
					...
					$whereUsua
					$whereTipoRadicado
					AND ".$db->conn->SQLDate('Y-m-d h:i:s', 'r.radi_fech_radi')." BETWEEN '$fecha_ini'  AND '$fecha_fin'
	...
	$queryEDetalle = "SELECT $radi_nume_radi as RADICADO, ".$db->conn->SQLDate('Y-m-d h:i:s', 'r.radi_fech_radi')." as FECHA_RADICACION,
		s.SGD_TPR_DESCRIP as TIPO_DOCUMENTO,
		".$db->conn->SQLDate('Y-m-d h:i:s', 'h.HIST_FECH')." as FECHA_HISTORICO, h.HIST_OBSE as OBSERVACION,
		 b.USUA_NOMB as USUARIO, d.DEPE_NOMB as DEPENDENCIA,r.radi_path as HID_RADI_PATH{$seguridad}
	...
	$queryEDetalle = "SELECT r.RADI_NUME_RADI as RADICADO, ".$db->conn->SQLDate('Y-m-d h:i:s', 'r.radi_fech_radi')." as FECHA_RADICACION,
		s.SGD_TPR_DESCRIP as TIPO_DOCUMENTO,
		".$db->conn->SQLDate('Y-m-d h:i:s', 'h.HIST_FECH')." as FECHA_HISTORICO, h.HIST_OBSE as OBSERVACION,
		 b.USUA_NOMB as USUARIO, d.DEPE_NOMB as DEPENDENCIA,r.radi_path as HID_RADI_PATH{$seguridad}
		FROM RADICADO r, HIST_EVENTOS h, DEPENDENCIA d, USUARIO b, 
		...
		WHERE 
			...
			AND h.DEPE_CODI = d.DEPE_CODI
			AND TO_CHAR(r.RADI_FECH_RADI,'yyyy/mm/dd') BETWEEN '$fecha_ini'  AND '$fecha_fin'
			AND s.SGD_TPR_CODIGO = r.TDOC_CODI $resolucion";
...
if(isset($_GET['genDetalle'])&& $_GET['genDetalle']==1)
		$titulos=array("#","1#RADICADO","2#FECHA DE RADICACI&Oacute;N","3#TIPO DE DOCUMENTO","4#FECHA HISTORICO","5#OBSERVACIONES","6#USUARIO","7#DEPENDENCIA");
	else 		
