...
{	case 'mssql':
	...
	case 'oci8':
        case 'oci8po':
        $sql="SELECT  FROM RADICADO r INNER JOIN dependencia d on r.radi_depe_actu=d.depe_codi INNER JOIN USUARIO u on u.usua_codi = r.radi_usua_actu and r.radi_depe_actu=u.depe_codi WHERE RADI_NUME_RADI = '".$buscar_por_radicado."'"; 
	...
	{$query1 = "
		...
		FROM RADICADO a,SGD_TPR_TPDCUMENTO b, SGD_DIR_DRECCIONES d, SGD_EXP_EXPEDIENTE g,
                dependencia d , USUARIO u 
		WHERE
		a.radi_nume_radi =d.radi_nume_radi AND
		a.radi_nume_radi  =g.radi_nume_radi (+) AND 
		a.tdoc_codi=b.sgd_tpr_codigo
                AND a.radi_depe_actu=d.depe_codi AND 
                 u.usua_codi = a.radi_usua_actu AND a.radi_depe_actu=u.depe_codi
		$whereTrd
		$where_ciu
		$where_general
		and rownum <= 200
