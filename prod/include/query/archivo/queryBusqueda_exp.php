...
$sqlFecha = $db->conn->SQLDate("Y-m-d h:i:s a","r.radi_fech_radi");
	...
	,e.SGD_EXP_CARPETA, e.SGD_EXP_RETE,$sqlFecha as FECH,e.sgd_exp_entrepa,e.sgd_exp_edificio, r.radi_path,r.RADI_CUENTAI,r.EESP_CODI, s.SGD_SEXP_NOMBRE, s.SGD_SEXP_ASUNTO
	 from SGD_SEXP_SECEXPEDIENTES s, SGD_EXP_EXPEDIENTE e, radicado r, SGD_DIR_DRECCIONES c where ";
	$sql.=" e.SGD_EXP_NUMERO=s.SGD_EXP_NUMERO and r.radi_nume_radi=e.radi_nume_radi and e.SGD_EXP_ESTADO='1' and c.RADI_NUME_RADI=e.RADI_NUME_RADI";
	//if($docu1==3 and $buscar_docu!="")$sql.=" and r.RADI_CUENTAI like '%$buscar_docu%' ";
	if($buscar_docu!="")
	{
		$buscar_docu= strtoupper($buscar_docu);
		$sql.=" and (upper(s.SGD_SEXP_NOMBRE) like '%$buscar_docu%' OR upper(s.SGD_SEXP_ASUNTO) like '%$buscar_docu%') ";
	}
	...
	if($buscar_docu!="")$sqlca.=",s.SGD_SEXP_NOMBRE,s.SGD_SEXP_ASUNTO";
	$sqlca.=" from RADICADO r,SGD_DIR_DRECCIONES c";
	if($buscar_docu!="")$sqlca.=",SGD_SEXP_SECEXPEDIENTES s";
	$sqlca.=" where ";
	if($buscar_docu!=""){
		$buscar_docu= strtoupper($buscar_docu);
		$sqlca.="(upper(s.SGD_SEXP_NOMBRE) LIKE '%$buscar_docu%' OR upper(s.SGD_SEXP_ASUNTO) LIKE '%$buscar_docu%') and c.RADI_NUME_RADI=r.RADI_NUME_RADI";
	}
	...
	where $srds $c $sbrds $d $pross $ef $r $edi $edifi $b $entre $s $caja $t $caja2 $t1 $entre $t2 $estan $t3 $ufisica $u $foli $k $fecha $i $fechafin $j $param $l $conse $n $depa $p $muni $q $x $a s.SGD_EXP_NUMERO=e.SGD_EXP_NUMERO and cast(e.RADI_NUME_RADI as varchar) LIKE '$radi' and e.SGD_EXP_ESTADO='1'";
 	
	$sql1="select SGD_EIT_NOMBRE from SGD_EIT_ITEMS where SGD_EIT_COD_PADRE like '$exp_edificio' order by SGD_EIT_NOMBRE";
	...
	$sql12="select SGD_EIT_NOMBRE from SGD_EIT_ITEMS where SGD_EIT_COD_PADRE like '$buscar_piso' order by SGD_EIT_NOMBRE";
 	$sql2="select SGD_EIT_SIGLA from SGD_EIT_ITEMS where SGD_EIT_CODIGO like '$ite1'";
 	$sql3="select SGD_EIT_SIGLA from SGD_EIT_ITEMS where SGD_EIT_CODIGO like '$ite2'";
 	$sql4="select SGD_EIT_SIGLA from SGD_EIT_ITEMS where SGD_EIT_CODIGO like '$ite3'";
 	if($ite4)$sql5="select SGD_EIT_SIGLA from SGD_EIT_ITEMS where SGD_EIT_CODIGO like '$ite4'";
	if($ite5)$sql10="select SGD_EIT_SIGLA from SGD_EIT_ITEMS where SGD_EIT_CODIGO like '$ite5'";
	if($ite6)$sql11="select SGD_EIT_SIGLA from SGD_EIT_ITEMS where SGD_EIT_CODIGO like '$ite6'";
	$sql13="select SGD_EIT_SIGLA from SGD_EIT_ITEMS where SGD_EIT_CODIGO like '$edificio'";
	$sql14="select SGD_EIT_SIGLA from SGD_EIT_ITEMS where SGD_EIT_CODIGO like '$bus'";
