...
switch( $db->driver )
	...
	case 'ocipo':
	case 'postgres':	
        $queryUs  = "SELECT DISTINCT( R.RADI_NUME_RADI ) AS \"".utf8_encode("IDT_Número Radicado")."\",
        ...
        if( $tradSel != 0 )
        {
            $queryUs .= " AND ".$db->conn->substr."(cast(R.RADI_NUME_RADI as varchar(100)), 14, 1 ) = '".$tradSel."'";
        }
