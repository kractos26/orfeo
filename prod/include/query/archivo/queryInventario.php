<?
	...
	$sqpl3="select count(sgd_eit_codigo) as COU from sgd_eit_items where sgd_eit_nombre like 'ESTANTE%'";
		...
		$sqltsb="select sgd_sbrd_descrip,sgd_sbrd_tiemag,sgd_sbrd_dispfin from sgd_sbrd_subserierd where sgd_sbrd_codigo=$sbrd and sgd_srd_codigo=$srd";
                ...
                $concatEdif=$db->conn->Concat("SGD_EIT_SIGLA","'-'","SGD_EIT_NOMBRE");

		$sql7="select $concatEdif,SGD_EIT_CODIGO from SGD_EIT_ITEMS WHERE SGD_EIT_COD_PADRE = 0 order by SGD_EIT_SIGLA";
		...
		$sqliw="select count(sgd_eit_nombre) AS CO from sgd_eit_items where sgd_eit_cod_padre like '$item1_cod'";
		$sqli1="select count(sgd_eit_nombre) AS CO from sgd_eit_items where sgd_eit_cod_padre like '$item2_cod'";
		$sqli2="select count(sgd_eit_nombre) AS CO from sgd_eit_items where sgd_eit_cod_padre like '$item3_cod'";
		$sqli3="select count(sgd_eit_nombre) AS CO from sgd_eit_items where sgd_eit_cod_padre like '$item4_cod'";
		$sqli4="select count(sgd_eit_nombre) AS CO from sgd_eit_items where sgd_eit_cod_padre like '$pisocod'";
 	...
 	and s.sgd_srd_fechfin order by detalle ";
		...
		$sqlo2='SELECT SGD_EIT_CODIGO AS "CODIGO", SGD_EIT_NOMBRE AS "NOMBRE"
		  ...
		  AND ( SGD_EIT_CODIGO = SGD_EXP_CAJA
		  OR SGD_EIT_CODIGO = SGD_EXP_ENTREPA )
		  OR ( SGD_EIT_CODIGO = SGD_EXP_CAJA
