...
function Header()
{
	...
	$this->SetXY($l,20+$margen);
 	$this->Cell(400,14,strtoupper($this->entidad_largo),0,0,'L');
	$this->SetXY($l,44+$margen);
 	$this->Cell(400,13,strtoupper($this->dependencia),0,0,'L');
	...
	if($ii<=9)
		...
		$ii++;
			...
			$this->Cell(0,$this->FontSizePt,trim($this->titleText),0,0,'C');
			$l = ($this->lMargin);
	...
	$ii=0;
		...
		{		
			$this->SetXY($l,$this->tMargin+10+$margen);
			$this->cell($this->tablewidths[$col],$maxY-($this->tMargin+30),'',0,0,'C',1);
			$this->SetXY($l,$this->tMargin+10+$margen);
...
function morepagestable($db,$lineheight=15,$query,$row_ini,$row_fin) {
		...
		if($i_datos>=$row_ini and $i_datos<=($row_fin))
			{
				...
				*/
				//$gp    = $this->rs->fields[0];    //	   $gp = trim(ora_getcolumn($this->cursor,14));
				$gp1   = $this->rs->fields[0]; //  $gp1 =trim(ora_getcolumn($this->cursor,2));
				$gptot = $this->rs->fields[0];	 //  $gptot = trim(ora_getcolumn($this->cursor,13));
		
				$registro++;
				if($gptot!=0)
				{		
					for($col=0;$col<$this->rs->FieldCount();$col++) //  for($col=0;$col<ora_numcols($this->cursor)-1;$col++)
					{	
					$txt = $this->rs->fields[$col];	//$txt = ora_getcolumn ($this->cursor,$col);
					if($col==2)  $txt .= $grupo;
					if($col==8)  $subtotal = $txt + $subtotal;
