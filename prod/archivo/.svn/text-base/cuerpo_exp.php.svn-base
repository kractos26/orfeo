...
// Comienza el ciclo para mostrar los documentos de la carpeta predeterminada.
   ...
   while(!$rs->EOF)
	...
	$rs2=$db->conn->Execute($tm6);
	$cj2=$rs2->fields['SGD_EIT_SIGLA'];
	if(($caja=="" or $caja==0) and $entre!="")$bus=$entre;
	else $bus=$caja;
	//$db->conn->Debug=true;
	...
	if($it7 and $it7==$edifi){$ite1=$it6;$ite2=$it5;$ite3=$it4;$ite4=$it3;$ite5=$it2;$ite6=$it1;}
	if($it6 and $it6==$edifi){$ite1=$it5;$ite2=$it4;$ite3=$it3;$ite4=$it2;$ite5=$it1;}
	if($it5 and $it5==$edifi){$ite1=$it4;$ite2=$it3;$ite3=$it2;$ite4=$it1;}		
	if($it4 and $it4==$edifi){$ite1=$it3;$ite2=$it2;$ite3=$it1;}


	if($edifi=="" and $entre=="" and $caja=="")$ubicacion="";
	else{
		$ubicacion=$ed;
		include "$ruta_raiz/include/query/expediente/queryCuerpo_exp.php";
		$rs5=$db->conn->Execute($tm4);
		if($ite1!=""){$zo=$rs5->fields['SGD_EIT_SIGLA'];$ubicacion.="-".$zo;}
		$rs4=$db->conn->Execute($tm3);
		if($ite2!=""){$ca=$rs4->fields['SGD_EIT_SIGLA'];$ubicacion.="-".$ca;}
		$rs2=$db->conn->Execute($tm1);
		if($ite3!=""){$es=$rs2->fields['SGD_EIT_SIGLA'];$ubicacion.="-".$es;}
		$rs3=$db->conn->Execute($tm2);
		if($ite4!=""){$et=$rs3->fields['SGD_EIT_SIGLA'];$ubicacion.="-".$et;}
		$rs6=$db->conn->Execute($tm5);
		if($ite5!=""){$cj=$rs6->fields['SGD_EIT_SIGLA'];$ubicacion.="-".$cj;}
		if($caja!="" or $caja ==0)$ubicacion.="-".$cj2;
	}
	include "$ruta_raiz/include/query/expediente/queryCuerpo_exp.php";
	//$db->conn->debug=true;
	$rsd=$db->query($sqlr);
...
if($estado==2 and $estado_nomb=="Si"){
	 if($rs->fields["RADI_PATH"]!="")$urlimagen = "<a href='../bodega".$rs->fields["RADI_PATH"]."?fechah=$fechah'><span class='".$class."'>$data</span></a>";
	 else $urlimagen = "<span class='".$class."'>$data</span>";
	 ?>
        <tr class='<?=$leido?>'>
	 <?
