...
if($grbNivel and $numRad)
{
	...
	if($nivelRad==1){
		...
		$observa = "Radicado Confidencial(Privado: Visible solo Usuario Actual)";
		$borrarTodo=true;
	}
	if($nivelRad==2){
		$query = "UPDATE RADICADO SET SGD_SPUB_CODIGO=2, SGD_SPUB_DEPE=$dependencia where radi_nume_radi=$numRad";
		$observa = "Radicado Confidencial(Dependencia: Visible solo Usuarios de la dependencia que asign&oacute; confidencialidad)";
		$borrarTodo=true;
	}
	if($nivelRad==3){
		$query = "UPDATE RADICADO SET SGD_SPUB_CODIGO=3 where radi_nume_radi=$numRad";
		$observa = "Radicado Confidencial(Visible solo Usuario(s) Espec&iacute;fico(s))";
		foreach ($selUsuPriv as $login)
		{
			$insertUsuarios="insert into sgd_matriz_nivelrad (radi_nume_radi, usua_login) values($numRad,'$login')";
...
$slcNivel="<select name='nivelRad' id='nivelRad' class='select' onchange='activa();'>";
foreach($arrNivel as $j=>$value)
{
	$nivelRad==$j?$est='selected':$est='';
...
$selectDep= $rs->GetMenu2('selDepPriv[]',$usDefault,false,true,3," id='selDepPriv' class='select' onChange='verUsuarios($numRad);' ");

//Se borra usuario
if($borrar)
{
	$sqlDel="delete from sgd_matriz_nivelrad where usua_login='$borrar' and radi_nume_radi=$numRad";
...
if($borrarTodo)
{
	$sqlDel="delete from sgd_matriz_nivelrad where radi_nume_radi=$numRad";
	$rs1=$db->conn->Execute($sqlDel);
}
...
if($nivelRad==3)
{
	...
	while(!$rs3->EOF)
    {
    	$usuariosNv .="   <tr>
                              ...
                              <font size=1>
                    	   	  <a href='radicado.php?borrar=".$rs3->fields["USUA_LOGIN"]."&numRad=$numRad&nivelRad=$nivelRad'>Borrar</a>
                       		  </font>
                              </td>
                           </tr>";
		$rs3->MoveNext();
...
function verUsuarios(numRad)
{
	if (xmlHttp)
	{
		...
		numRad = encodeURIComponent(numRad);
		var tipo = "deps="+vl2+"&i=2&numRad="+numRad;
		cache.push(tipo);
...
function activa()
{
	...
	if(document.getElementById('nivelRad').value==2)
	{
	...
	if(document.getElementById('nivelRad').value==3)
	{
...
<tr align="center" class="titulos2">
	<td height="15" class="titulos2" colspan="2" >NIVEL DE SEGURIDAD DEL RADICADO No. <?=$numRad?></td>
</tr>
<tr >
