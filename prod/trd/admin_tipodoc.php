...
$encabezadol = $_SERVER['PHP_SELF']."?krd=$krd&terminot=$terminot&codusua=$codusua&depende=$depende&ent=$ent";
...
{	$i = 0;
	$cad = "perm_tp";
	$tipos = "<tr><td height='26' class='listado2' width='100%'>";
	$cmp = "SGD_TPR_TP";
...
<tr>
	...
	<TD valign="top" align="left" class='listado2'>
		<input type=text name=detatipod value='<?=$detatipod?>' class='tex_area' size=50 maxlength="75" >
	</td>
</tr>
<tr> 
...
<tr>
	<td height="26" colspan="5" valign="top" class='titulos2'>
		<center>
...
{	$whereBusqueda = " where upper(sgd_tpr_descrip) like '%".strtoupper($detatipod)."%'";	}
...
{	$isqlB = "select * from sgd_tpr_tpdcumento where upper(rtrim(sgd_tpr_descrip)) = '$detatipod' "; 
	...
	{	$isql = "select max(sgd_tpr_codigo) as NUME from sgd_tpr_tpdcumento"; 
		...
		$query="insert into SGD_TPR_TPDCUMENTO(SGD_TPR_CODIGO, SGD_TPR_DESCRIP,SGD_TPR_TERMINO,".$ins_cmp.")".
				"VALUES ('$radiNumero','$detatipod','$terminot',".$ins_vlr.")";		
...
{	
	...
	{	
		...
		$query = "update SGD_TPR_TPDCUMENTO set SGD_TPR_DESCRIP ='$detatipod',$cadena
				SGD_TPR_TERMINO = '$terminot', SGD_TERMINO_REAL = '$terminot'  where  sgd_tpr_codigo = $codtdocI ";		
		$rsIN = $db->conn->query($query);
		$terminot = '' ;
		$detatipod = '';
		$cadena = '';
