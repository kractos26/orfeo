...
// Inicializa, oculta o presenta los par�metros de b�squeda dependiendo de la opci�n del men� de pr�stamos seleccionada
   ...
   $opcionMenu=strip(get_param("opcionMenu")); //opci�n seleccionada del men�    
...
//===============================
   function Search_Show(){  
      ...
      if ($v_hora_limite=="")  { $v_hora_limite  =strip(get_param("s_hora_limite")); }
      if ($v_minuto_limite==""){ $v_minuto_limite=strip(get_param("s_minuto_limite")); }
      if ($v_meridiano=="")    { $v_meridiano    =strip(get_param("s_meridiano")); }	  
...
<?                  $validUsuaActiv="";
                    if ($opcionMenu==1) { $validUsuaActiv=" USUA_ESTA='1' "; }ELSE { $validUsuaActiv=" USUA_LOGIN IS NOT NULL "; } //Verifica que el usuario se encuentre activo para hacer el pr�stamo                                
