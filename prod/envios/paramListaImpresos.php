...
<body>
    ...
    <script language="javascript">
        ...
        function cancela()
        {
            url="<?= $ruta_raiz ?>/envios/cuerpoMarcaEnviar.php?<?= "krd=$krd&" . session_name() . "=" . trim(session_id()) ?>";
            window.location=url;
        }
    </script>
...
<tr>
    <td height="26" class='titulos2'>Tipo de radicaci&oacute;n</td>
    <td class="listado5"> 
        <?
...
<tr>
    ...
    <td class="listado5"> 
        ...
        if ($_SESSION["usua_perm_impresion"] == 2) {
             ...
             */
            $DepFilt = " where depe_codi in (select depe_codi from impusrrel where usua_doc='" . $_SESSION["usua_doc"] . "') ";
        }
        $ss_Default = "--- Todos las Dependencias ---";
        $valor = 999;
        $sqlDep = "select depe_nomb,depe_codi from dependencia $DepFilt order by 1";
        $rsDep = $db->conn->Execute($sqlDep);
        print $rsDep->GetMenu2("DepeCod", "$DepeCod", $blank1stItem = "$valor:$ss_Default", false, 0, " class='select'");
        ?>
    </td>
    </td>
</tr>
...
<center>
    ...
    <INPUT TYPE=BUTTON name=cancelarAnular value=Cancelar ONCLICK=" cancela();" class=botones_funcion>
    </td></tr>
    </table>
    <?php
    if (!$fecha_busq)
