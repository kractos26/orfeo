<?    session_start();
   ...
   $ruta_raiz = ".";
   	if (!$krd or !$_SESSION['dependencia'])   include "rec_session.php";
...
<?
	...
	$conexion = new ConnectionHandler;
	 $vector = array();
	 $i=0;
	 if($chk1){$vector[$i++] = $chk1;}
	 if($chk2){$vector[$i++] = $chk2;}
	 if($chk3){$vector[$i++] = $chk3;}
	 if($chk4){$vector[$i++] = $chk4;}
	 if($chk5){$vector[$i++] = $chk5;}
	 if($chk6){$vector[$i++] = $chk6;}
	 if($chk7){$vector[$i++] = $chk7;}
	 if($chk8){$vector[$i++] = $chk8;}
	 if($chk9){$vector[$i++] = $chk9;}
	 if($chk10){$vector[$i++] = $chk10;}
	 if($chk11){$vector[$i++] = $chk11;}
	 if($chk12){$vector[$i++] = $chk12;}
	 if($chk13){$vector[$i++] = $chk13;}
	 if($chk14){$vector[$i++] = $chk14;}
	 if($chk15){$vector[$i++] = $chk15;}
	 if($chk16){$vector[$i++] = $chk16;}
	 if($chk17){$vector[$i++] = $chk17;}
	 if($chk18){$vector[$i++] = $chk18;}
	 if($chk19){$vector[$i++] = $chk19;}
	 if($chk20){$vector[$i++] = $chk20;}
