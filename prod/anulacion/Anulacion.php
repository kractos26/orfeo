...
/**
 * 
 * Metodo encargado de realizar la solicitud de anulación.
 * @param array $radicados
 * @param smallint $dependencia
 * @param varchar $usuadoc
 * @param varchar $comentario
 * @param smallint $codUsuario
 * @param datetime $systemDate
 * @param smallint $idMotivo
 */
  function solAnulacion($radicados,  $dependencia,  $usuadoc, $comentario, $codUsuario, $systemDate, $idMotivo)
  {
...
# Codigo de prueba para UPDATE
		foreach($radicados as $noRadicado)
		{
			...
			# Insertamos en la tabla sgd_anu_anulados los registros que entran en solicitud.
				$sql = "insert into sgd_anu_anulados (RADI_NUME_RADI, SGD_EANU_CODI, SGD_ANU_SOL_FECH, DEPE_CODI, USUA_DOC, SGD_ANU_DESC ,USUA_CODI, MOTIVO_ANULACION_CODIGO) 
				        values ($noRadicado   ,1 , $systemDate ,
						$dependencia, $usuadoc , '$comentario', $codUsuario, $idMotivo)";
