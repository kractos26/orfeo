...
<TABLE width="100%" class='borde_tab' cellspacing="5">
  ...
  <form name="new_product"  action='anularRadicado.php?<?=session_name()."=".session_id()."&krd=$krd&fecha_h=$fechah"?>' method=post>
<center>
...
if($depeBuscada and $depeBuscada != 0)
{
  $whereDependencia = " b.DEPE_CODI=$depeBuscada AND";
	...
	include "$ruta_raiz/include/query/anulacion/queryanularRadicado.php";

	$whereTipoRadi=(!empty($_POST["tipoRadicado"]))?" AND r.radi_nume_radi like '%".$_POST["tipoRadicado"]."' ":" ";
	error_reporting(7);
	...
	$query = "select $radi_nume_radi as radi_nume_radi, r.radi_fech_radi, r.ra_asun, r.radi_usua_actu,
				r.radi_depe_actu, r.radi_usu_ante, c.depe_nomb,".$db->conn->SQLDate("Y-m-d H:m:i ","b.sgd_anu_sol_fech")." as sgd_anu_sol_fech, ".$db->conn->substr."(b.sgd_anu_desc, 21) as sgd_anu_desc
			 from radicado r inner join
                                sgd_anu_anulados b on r.radi_nume_radi=b.radi_nume_radi
                                inner join dependencia c on  b.depe_codi = c.depe_codi";
	...
	$where_isql = " WHERE $whereDependencia	b.sgd_anu_sol_fecH BETWEEN ".
						...
						" and SGD_EANU_CODI = 1 $whereTipoRadi";
	...
	require "../anulacion/class_control_anu1.php";
...
//
	 ...
	 */
		...
		foreach($radAnularE as $id=>$noRadicado)
		{
	  		$radicadosPdf .= "<tr>
					  <td>".$radAnularE[$id] ."&nbsp;&nbsp; &nbsp;</td>
					  <td>".htmlspecialchars($radObservaE[$id])."</td>
			 </tr>";
		}
		$anoActual = date("Y");
		$radicadosPdf .= "</table>";
	...
	$fecha_hoy =  $b->traducefecha($date);
  	$html = "
		...
		<CENTER><B>
		&nbsp;&nbsp; &nbsp;&nbsp; &nbsp;&nbsp;
		NUMEROS DE RADICACI&Oacute;N DE CORRESPONDENCIA ENVIADA  A&Ntilde;O $anno</B></CENTER><p>
		<CENTER><B>
		&nbsp;&nbsp; &nbsp;&nbsp; &nbsp;&nbsp; &nbsp;&nbsp;
		CENTRO DE RECEPCI&Oacute;N DE INFORMACI&Oacute;N Supertransporte </B></CENTER><p align='justify'>
		<br />
		...
		Coordinador  del Centro de Recepci&oacute;n de Informaci&oacute;n Supertransporte<br />
                 </p>
                 <p align='justify' >
		$entidad_largo procede a anular los siguientes n&uacute;meros de radicaci&oacute;n de $TituloActam que no fueron tramitados por las dependencias radicadoras:</p><p><br />
		...
		$usua_nomb <br />
		Centro de Recpci&oacute;n de Informaci&oacute;n Supertransporte.";
  	$html = 	'<img src="encabezadoSPT.jpg" width="520">' . $html;
  	$ruta_raiz = "..";
	$pdf=new PDF();
	$pdf->Open();
	...
	$pdf->WriteHTML($html);
	//save and redirect
	$noArchivo = "../poolsgc2013".$noArchivo;
	$pdf->Output($noArchivo);
	?>
		<center>Ver Acta <a href='<?=$noArchivo?>'>Acta No <?=$actaNo?> </a></center>
		<?php
    exit;
}
}
