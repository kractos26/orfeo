...
function validar(action)
{
  ...
  <?if($tipo_envio!=105){?>
   solonumeros();
   <?}
   else{?>document.new_product.submit();<?}?>
}
...
<table class=borde_tab width='100%' cellspacing="5"><tr><td class=titulos2><center>GENERACI&Oacute;N PLANILLAS Y GU&Iacute;AS DE CORREO</center></td></tr></table>
...
<TABLE width="450" class="borde_tab" cellspacing="5">
  ...
  <tr>
	...
	{
	case 'mssql':
            $concat=$db->conn->Concat("convert(char(4),sgd_fenv_codigo)","'-'"," sgd_fenv_descrip");
	break;
	case 'oracle':
	case 'oci8':
	case 'oci805':
        case 'oci8po':
        default:
            $concat=$db->conn->Concat("sgd_fenv_codigo","'-'"," sgd_fenv_descrip");
	}
        $sqlfenv="select  
                    ".$concat.",
                        sgd_fenv_codigo 
