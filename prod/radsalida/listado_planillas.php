...
// Si la variable $generar_listado_existente viene entonces este if genera la planilla existente
	...
	$head_table = array ("CANTIDAD",utf8_encode("CATEGORÍA DE CORRESPONDENCIA"),"RADICADO","DESTINATARIO","DESTINO","PESO EN GRAMOS","VALOR PORTE","CERTIFICADO","VALOR ASEGURADO","TASA DE SEGURO","VALOR REEMBOLSABLE","AVISO DE LLEGADA","SERVICIOS ESPECIALES","VALOR TOTAL PORTES Y TASAS");
	$head_table_size = array (57   ,90                            ,70                  ,234           ,104      ,53               ,44           ,74           ,72               ,50             ,83                  ,65                ,75                    ,80);
	...
	//$arpdf_tmp = "../bodega/pdfs/planillas/$dependencia_". date("Ymd_hms") . "_jhlc.pdf"; Comentariada Por HLP.
        ...
        $db=$db_old;
	$rutaPDF = "pdfs/planillas/".$dependencia."_".date("Ymd_hms")."_jhlc.pdf";
        $arpdf_tmp="$ruta_raiz/$carpetaBodega/".$rutaPDF;
	$pdf->SetFont('Arial','',8);
...
<a href='<?=$ruta_raiz?>/seguridadImagen.php?fec=<?=base64_encode($rutaPDF)?>' target='<?=date("dmYh").time("his")?>'>Abrir Archivo PDF</a></center>
</td>
</TR>
</TABLE>
</body>
