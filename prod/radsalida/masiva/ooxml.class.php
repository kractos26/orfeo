<?php
/**
 * Clase para manejar archivos DOCX ooxml
 *
 * @author  Jairo Losada  
 *          Adaptacion de OpenDocument REALIZADA por :cmauriciop@yahoo.com.mx
 * @version v1.0 01/02/2012
 */ 
 ...
 /* Variables privadas */
  ...
  var $debug = false;
	...
	function setWorkDir ($workDir) {
		$cambio = false;
    //exec( "rm -rf " . $workDir . "/" . $this->directorio . "*" );
	...
	{	$nombreDir = explode('.',$this->nombreOdt);
		...
		if (!$existeDir)
		{
			...
			echo $error;
			$creoDir = @mkdir($this->cacheDir,0777,true);
			...
			{	( $this->debug == true) ? ( $error = "<CENTER><table class=borde_tab><tr><td class=titulos2>Cre&oacute el directorio: " . $this->cacheDir . $nombreDir[0]. " todo ok</td></tr></table>" ) : $error = '' ;
				...
				default:
					...
					if($this->debug == true) echo "<hr>Verificando.. $verificacion <hr>";
				break;
			}
    ...
    function cargarContenido(){
		...
		if ($esDir){
          ...
          if ($this->fp3) {
            ...
            $this->_debug();
            if( $this->debug == true) $error = "<br><b>No existe el archivo de cabeceras.</b>";
            return false;
          }
...
if($esArchivoStyles)
{
	...
	if ($vieneTemp != null)
	{
		...
		if ($tipoUnitario == '1'){	
			...
			$cmd_cp = "cp " . $this->workDir.$nombreZip[0].".docx ".str_replace($this->barra."tmp".$this->barra."workDir", "",$this->workDir).str_replace(".".$this->barra,"", $archivoFinal);
			switch ($this->SO)
			{	case 'W':
				{
				...
				default: 
					...
					}else{	$cmd_cp = "cp " . $this->workDir . $nombreZip[0] . ".docx " . $workDIR .  str_replace('/',$this->barra,$archivoFinal);
						switch ($this->SO)
						{	case 'W':
