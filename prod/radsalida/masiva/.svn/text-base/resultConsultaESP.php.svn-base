<?
/**
 * Programa que muestra el resultado de la consulta de acuerdo a la selección realizada en consultaESP.php
...
{	case '0':
	{	//Si incluyó alguna cadena a buscar
		...
		$sqlWhere.="1=1 ";
		$sql = "SELECT nuir ID,nombre_de_la_empresa NOMBRE,sigla_de_la_empresa SIGLA,ID_CONT IDCONT,
				ID_PAIS IDPAIS, codigo_del_departamento IDDPTO, codigo_del_municipio IDMPIO
	...
	{	//Si incluyó alguna cadena a buscar
		...
		$sqlWhere.="1=1 ";
		$sql = "SELECT SGD_OEM_CODIGO ID,SGD_OEM_OEMPRESA NOMBRE,SGD_OEM_SIGLA SIGLA,ID_CONT IDCONT,
				ID_PAIS IDPAIS, DPTO_CODI IDDPTO, MUNI_CODI IDMPIO
	...
	{	//Si incluyó alguna cadena a buscar
		...
		$sqlWhere.="1=1 ";
		$sql = "SELECT SGD_CIU_CODIGO ID,SGD_CIU_NOMBRE NOMBRE,SGD_CIU_APELL1 SIGLA,ID_CONT IDCONT,
				ID_PAIS IDPAIS, DPTO_CODI IDDPTO, MUNI_CODI IDMPIO
...
/**
* Envía el formulario de acuerdo a la opción seleccionada
*/
...
/**
* Selecciona todos los registros del resultado de la búsqueda
*/
function markAll()
{
...
{	var hay_chk;	//Bandera para saber si hay seleccionados.
	var vec_selec;	//Acá guardaremos a los que han sido seleccionados.
...
{	if (tipo == 0)
	...
	{	if (valida_check(forma) == tipo)	enviar('selecConsultaESP.php');
		...
		{	// En caso de querer validar cambios en la seleccion de registros se haría acá.
			// Por ahora solo está para focalizar los envios para el mismo tipo de destino previo.
...
{	$i++;
	...
	if (strlen($rs->fields['ID']))
		//Si el registro ya fué seleccionado lo muestra marcado
...
?>
	<tr align="center">
		<td height="30" class='titulos2' colspan="8">
	        ...
	        <input name="seleccionar" type="SUBMIT"  class="botones" id="envia22"  value="Seleccionar" onClick="return valida_lugar(this.form,<?php ($_POST['tipo_masiva'] == '') ? print 0 : print $_POST['tipo_masiva'] ; ?>);">
