...
$params=$phpsession."&krd=$krd&dependencia=$dependencia&codiTRD=$codiTRD&depe_codi_territorial=$depe_codi_territorial&usua_nomb=$usua_nomb&tipo=$tipo&"
				."depe_nomb=$depe_nomb&usua_doc=$usua_doc&codusuario=$codusuario&ano=$ano&";
...
{
$dirActual = getcwd();
	if(!copy($archivoPlantilla, "../../$carpetaBodega/masiva/".$arcPlantilla))
	{
		echo "error al copiar Plantilla: $archivoPlantilla en ../../$carpetaBodega/masiva/".$arcPlantilla;
	}elseif (!copy($archivoCsv, "../../$carpetaBodega/masiva/".$arcCsv)) {
		echo "error al copiar CSV: $archivoCsv en ../../$carpetaBodega/masiva/" .$arcCsv;
	}
	else
	{
		...
		if( $extension == 'doc' ){
			$archivoFinal = "$carpetaBodega/$ano/$dependencia/docs/$usua_doc"."_$fecha"."_$hora.doc";
			$archivoTmp = "$carpetaBodega/masiva/tmp_$usua_doc"."_$fecha"."_$hora.doc";
		}else {

			$archivoFinal = "$carpetaBodega/$ano/$dependencia/docs/$usua_doc"."_$fecha"."_$hora.odt";
			$archivoTmp = "/masiva/tmp_$usua_doc"."_$fecha"."_$hora.odt";
		}
		...
		$fp=fopen("$ruta_raiz/$carpetaBodega/masiva/$archInsumo",'w');
	 	...
	 	{	fputs ($fp,"plantilla=$arcPlantilla"."\n");
			...
			fputs ($fp,"archTmp=$carpetaBodega/$archivoTmp"."\n");
			fclose($fp);
	 	}
	 	else
...
//			$estadoTransaccion!=0 &&
			echo ("<br>$archInsumo");
				...
				{	echo ("<BR><span class='info'> Por favor guarde el archivo y verifique que los datos de combinacion  esten correctos <br>");
					echo ("<a class='vinculos' href=javascript:abrirArchivoaux('$archivoTmp')>Guardar Archivo </a></span> ");
					echo ("<br><br>");
					echo( "<br><input name='enviaDef' type='button'  class='botones' id='envia22'  onClick='enviar()' value='Generar Definitivo'>");
					echo( "<input name='cancel' type='button'  class='botones' id='envia22'  onClick='cancelar()' value='Cancelar'>");
				}
