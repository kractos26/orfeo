...
if (trim(strlen($grupo))){
	...
	while ($i < $num) {	
        ...
        $nombPdf =iconv($odt->codificacion($datosRad["nombre"]),'ISO-8859-1',$datosRad["nombre"]);
        $dirPdf  =iconv($odt->codificacion($datosRad["direccion"]),'ISO-8859-1',$datosRad["direccion"]);
        $dptoPdf =iconv($odt->codificacion($datosRad["deptoNombre"]),'ISO-8859-1',$datosRad["deptoNombre"]);
        $muniPdf =iconv($odt->codificacion($datosRad["muniNombre"]),'ISO-8859-1',$datosRad["muniNombre"]);
		$data=  array_merge ($data,array (array('NUMERO RADICADO'=>$radsGrupo[$i],'FECHA RADICADO'=>$rad->getRadi_fech_radi(),'DESTINATARIO'=>$nombPdf,'DIRECCION'=>$dirPdf,'DEPARTAMENTO'=>$dptoPdf,'MUNICIPIO'=>$muniPdf,'ELIMINADO'=>$eliminado)));
		$i++; 
	}
	
}else{
	$data =  array_merge ($data,array (array('RESULTADO'=>"NO SE ESPECIFIC� EL GRUPO A RECUPERAR")));				
}
...
$archivo = "masiva/tmp_$usua_doc"."_$fecha"."_$hora.pdf";
//Referencia el PDF a generar
$pdfcode = $pdf->ezOutput();
include("$ruta_raiz/config.php");
$fp=fopen("$ruta_raiz/$carpetaBodega/".$archivo,'wb'); 
fwrite($fp,$pdfcode); 
fclose($fp);
?>                	 	
      ...
      <table width="100%" border="0" cellpadding="2" cellspacing="0">
        <tr align="left"> 
          <td height="84" class="listado2_no_identa" >Se ha generado el archivo con el grupo <?=$grupo?>, recuperado.<br>
            ...
            al archivo: <a href="<?="$ruta_raiz/seguridadImagen.php?fec=".base64_encode($archivo)?>"> PDF GENERADO</a>.</span></strong></font></td>
        </tr>
      </table>
      </td>
      </tr>
