...
{	case '0':
	{	$tipo_consulta="ENTIDADES";
		...
		if (strlen($nombre)) $sqlWhere.=" (upper(nombre_de_la_empresa) like '%$nombre%' or upper(sigla_de_la_empresa) like '%$nombre%') and ";
...
{	if (strlen(${$tmp_cad1.$i}) >0)
	{	${"vsel".$i} = explode(',',${$tmp_cad1.$i});
		...
		unset(${'vsel'.$i});unset(${'vselct'.$i});		
	}
}
?>
function markCheck(idEmpresa)
{
	var hayAlgunoSeleccionado = false;
	var tempo = document.getElementById('slc_ctt['+idEmpresa+']');
	var namecheck = 'check_value['+idEmpresa+']';
	var tmp = document.getElementsByName(namecheck);
	
	for (var i = 0; i<tempo.length; i++)
	{
		if(tempo[i].selected && !hayAlgunoSeleccionado)
	...
	{
		if (!tmp[0].checked)	tmp[0].checked = true;
	}
	else	tmp[0].checked = false;
}

function selecCtt(idEmpresa)
{
	var hayAlgunoSeleccionado = false;
	var tempo = document.getElementById('slc_ctt['+idEmpresa+']');
	var namecheck = document.getElementsByName('check_value['+idEmpresa+']');
	for (var i = 0; i<tempo.length; i++)
	{
		if(tempo[i].selected && !hayAlgunoSeleccionado)
		{	hayAlgunoSeleccionado = true;
	...
	if (hayAlgunoSeleccionado)
	{
		if (confirm('Esta empresa tiene contactos seleccionados, al des-seleccionarla \ntambien des-seleccionara los contactos.\n\nDesea remover esta empresa de la selecci\xf3n....'))
		{
			for (var i = 0; i<tempo.length; i++)
			{
				if(tempo[i].selected)
				{	
					tempo[i].selected = false;
				}
			}
		}
		else namecheck[0].checked  = true;
	}
}
</script>
</head>
...
<table width="52%" border="0" cellspacing="5" align="center" class="borde_tab">
	<tr align="center">
		<td height="25" class="titulos2" colspan="8">RADICACI&Oacute;N MASIVA <BR>CONSULTA DE <?echo $tipo_consulta?></td>
    </tr>
    <tr align="center">
...
$i=-1;
$vec_id = array();
$vectorCod = explode(',',${'selected'.$_POST['slc_tb']});
$vectorCtt = explode(',',${'selectedctt'.$_POST['slc_tb']});
while ($rs&&!$rs->EOF)
{	$i++;
	...
	if (strlen($rs->fields['ID']))
		...
		{	$chequeado="checked";
			...
			$posIdContCiud = array_keys($vectorCod, $rs->fields['ID'], true);	//Trae la posicion del ID en el "vector" $selected
...
?>
	<tr align="justify" class="<?=$clase?>">
		...
		<td class='listado2' height="12" width="7%" align="center" >
			...
			<CENTER><input type="checkbox" id="check_value"  value="selec"  <?php echo "$chequeado NAME=check_value[".$rs->fields['ID']."]" ?> onclick="selecCtt(<? echo $rs->fields['ID'] ?>)"></CENTER>
		</td>
		<?php
