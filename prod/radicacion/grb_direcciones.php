...
$record = array();
if(!$med)$med=0;
$sqlEmpty = "select sgd_dir_codigo from sgd_dir_drecciones where radi_nume_radi=$nurad and sgd_dir_tipo=1";
$rsEmpty=$conexion->conn->Execute($sqlEmpty);
if((!$modificar  and !$rege and !$_POST['Submit4']) || ($rsEmpty && $rsEmpty->EOF))
{
...
if($nextval>=1) $record['SGD_DIR_CODIGO'] = $nextval;
...
if($cc)
{
	...
	$sgd_esp_codigo='null';
	$sgd_fun_codigo='null';
        if($idcont1 == 0 or $idcont1 == "")$idcont1=$selCont;
        if($selPais)$idpais1=$selPais;
        if($selDepto)$dpto_tmp1=$selDepto;
        if($selMnpio)$muni_tmp1=$selMnpio;
	...
	if($tipo_emp_us1==6){
        ...
        if(!$sgd_dir_codigo_cc){
            if($newId==true)
                    ...
                    {
                            //$db->conn->RollbackTrans();
                            echo "<span class='etextomenu'>No se encontrasena la secuencia sec_dir_direcciones ";
                    }
          ...
          $isql = "insert into SGD_DIR_DRECCIONES (SGD_TRD_CODIGO, SGD_DIR_NOMREMDES, SGD_DIR_DOC, MUNI_CODI, DPTO_CODI,
                        id_pais, id_cont, SGD_DOC_FUN, SGD_OEM_CODIGO, SGD_CIU_CODIGO, SGD_ESP_CODI, RADI_NUME_RADI, SGD_SEC_CODIGO,
                        SGD_DIR_DIRECCION, SGD_DIR_TELEFONO, SGD_DIR_MAIL, SGD_DIR_TIPO, SGD_DIR_CODIGO, SGD_ANEX_CODIGO, SGD_DIR_NOMBRE,MREC_CODI) ";
          $isql .= "values ('$sgdTrd', '$grbNombresUs1', '$cc_documento_us1', $muni_tmp1, $dpto_tmp1, $idpais1, $idcont1,
                            ...
                            '".trim($telefono_us1)."', '$mail_us1', $sgd_dir_tipo, $nextval, '$codigo', '$otro_us7',$med )";
          $dir_codigo_new = $nextval;
          $nextval++;
          $rsg=$conexion->query($isql);
          if (!$rsg)
        ...
        {
            $sqlUp="UPDATE SGD_DIR_DRECCIONES SET
		SGD_DIR_NOMREMDES = '".trim($nombre_us1)."',
		MUNI_CODI= ". $muni_tmp1.",
		DPTO_CODI= ". $dpto_tmp1.",
		ID_PAIS= ".$idpais1.",
		ID_CONT= ". $idcont1.",
		SGD_DIR_DIRECCION= '". $direccion_us1."',
		SGD_DIR_TELEFONO= '". trim($telefono_us1)."',
		SGD_DIR_MAIL= '".$mail_us1."',
		SGD_DIR_NOMBRE= '$otro_us7',
                MREC_CODI= ".$med.",
                SGD_DOC_FUN=$sgd_fun_codigo,
                SGD_OEM_CODIGO=$sgd_oem_codigo,
                SGD_CIU_CODIGO=$sgd_ciu_codigo, 
                SGD_ESP_CODI=$sgd_esp_codigo
                WHERE
                SGD_DIR_CODIGO=$sgd_dir_codigo_cc";
                $rsg=$conexion->query($sqlUp);
		if (!$rsg)
                    $error=12;
