<?
	...
	$encabezadol = "$PHP_SELF?".session_name()."=".session_id()."&krd=$krd&nurad=$nurad&coddepe=$coddepe&codusuario=$codusua&codusua=$codusua&codusuario=$codusuario&depende=$depende&ent=$ent&tdoc=$tdoc&codiTRDModi=$codiTRDModi&codiTRDEli=$codiTRDEli&codserie=$coddocu&tsub=$tsub&ind_ProcAnex=$ind_ProcAnex";
  ...
  <?
  	...
  	if ($coddocu != 0) {
  		if($coddocu > 10000){
  			...
  			$anexMod = $trd->actualizarTRDAUnitario( $nurad, $coddocu );
  			if($anexMod)$resultadoActualizacion = "Se actualiz&oacute; el Anexo: " . $nurad . ", con el tipo documental: " . $coddocu;
            else$mensaje_err="no se pudo insetar!!";
  		}
	...
	echo $rsD->GetMenu2("coddocu", $coddocu, "0:-- Seleccione --", false,"","onChange='submit()' class='select'" );
 ?>   
      </td>
     </tr>
