...
/**
 * Esp es la clase encargada de gestionar las operaciones y los datos bï¿½sicos referentes a una Esp
...
*/
 ...
 /**
  * Carga los datos de la instacia con con  referencia a un cï¿½digo de ESP suministrado retorna falso si no lo encuentra, de lo contrario true
  * @param	$codigo	string es el cï¿½digo del departamento
	* @return   boolean
  */
...
/**
 * Genera un archivo CSV con la informaciï¿½n de las empresas que recibe como parï¿½metro
...
{ 	if (${'s'.$i})		//Si se envio el vector....
	{
		...
		//variable que almacena la cantidad de empresas que llegarom como parï¿½metro
		$num = count($arrEmpresas);
		$x = 0;

		//Se recorre el arreglo de las empresas para armar el IN dentro del where de la bï¿½squeda
		while ($x < $num)
		{
			$nuir=$arrEmpresas[$x];
...
/**
 * Genera un archivo PDF con la informaciï¿½n de las empresas que recibe como parï¿½metro
 * @param	$s(0/1/2/)	string	con las (ESP/Empresas/Ciudadanos) seleccionadas
...
{ 	if (${'s'.$i})		//Si se envio el vector....
	{
		...
		{	$y++;
			...
			$nombESP = iconv("UTF-8","ISO-8859-1",$rs->fields['PPAL']);
			$dirESP = iconv("UTF-8","ISO-8859-1",$rs->fields['DIR']);
			$nuir =  iconv("UTF-8","ISO-8859-1",$rs->fields['ID']);
			$nomRepESP = iconv("UTF-8","ISO-8859-1",$rs->fields['DESCR']);
			...
			$contenido= $contenido ."$i,$com$nombESP$com,$com$dirESP$com,$com$nombMuni$com,$com$nombDepto$com,$com$nombPais$com,$com$nuir$com,$com$nomRepESP$com,$com$dignatario$com,$com$cargo$com\n";
			$data =  array_merge ($data,array (array('#'=>$y,'Tipo'=>$i,'Nombre'=>$nombESP,'Dirección'=>$dirESP,'País'=>$nombPais,'Departamento'=>$nombDepto,'Municipio'=>$nombMuni,'Nuir'=>$nuir,'Representante'=>$nomRepESP,'Dignatario'=>$dignatario,'Cargo'=>$cargo)));
			$rs->MoveNext();
	}	}	}
