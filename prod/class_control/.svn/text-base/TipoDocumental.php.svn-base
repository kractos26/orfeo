	...
	if ($tip_trd == $tip_dcto)
	    {
		 ...
		 $isqlM = "select $radi_nume_radi AS RADI_NUME_RADI,
	                SGD_MRD_CODIGO
...
{		require("../include/query/busqueda/busquedaPiloto1.php");
		...
		foreach($radicados as $noRadicado)
		{
			...
			$uSQL = "UPDATE RADICADO SET FECH_VCMTO =  ".$db3->conn->DBDate('Y-m-d', $fecVenEval)." WHERE radi_nume_radi = $fldRADI_NUME_RADI ";
			$db3->query($uSQL);
