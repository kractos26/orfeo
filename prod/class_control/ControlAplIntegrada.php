...
class  ControlAplIntegrada{
  ...
  * @param	$codigo	string es el c�digo del radicado
  ...
  * retora 1 y la ejecución fué exitosa o si no tiene querys adicionales por ejecutar, de lo contrario retorna 0
  * @param	$codigo	string radicao involucrado
		...
		while  (!$rs->EOF && $rs)	{
			...
			if (!$rs2){
			 	$this->cursor->conn->RollbackTrans();
		 	 	echo ("<span class='alarmas'>ERROR TRATANDO DE EJECUTAR LAS ACTUALIZACIONES ADICIONALES PARAMETRIZADAS PARA EL PLICATIVO ASOCIADO...".$aplRad['nombre']." en ($q)</span>");
		 	 	return ("0");
			}
			$rs->MoveNext();
		}
