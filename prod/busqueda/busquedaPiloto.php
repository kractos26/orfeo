...
$(document).ready(function() {
	$(".botonExcel").click(function(event) {
		$("#datos").val( $("<div>").append( $("#ExporExcel").eq(0).clone()).html());
		$("#FormExport").submit();
});
});
</script>
...
<html>
        ...
        function limpiar()
    {
         ...
         * Fecha de modificaciï¿½n: 30-Junio-2006
...
{
    if(tip==0)
        alert ("No tiene permiso para ver el radico porque no es su rol");
    if(tip==1)
        ...
        <table>
            <tr>
                <td valign="top">
                    ...
                    if ($_POST["Busqueda"] or $s_entrada) {
    ...
    <form method="post" action="<?= $sActionFileName ?>?<?= session_name() . "=" . session_id() ?>&indiVinculo=<?= $indiVinculo ?>&verrad=<?= $verrad ?>&carpeAnt=<?= $carpeAnt ?>&nomcarpeta=<?= $nomcarpeta ?>&dependencia=<?= $dependencia ?>&krd=<?= $krd ?>" name="Search">
        ...
        <table  border=0 cellpadding=0 cellspacing=2 class='borde_tab'>
            ...
            */
            ...
            <tr>
                <td class="titulos5">N° Guia</td>
                <td class="listado5">
                    <input class="tex_area" type="text" name="s_GUIA" maxlength="" value="" size="20" >
                </td>
            </tr>
...
//-------------------------------
    if ($indiVinculo != 2) {
        ?>
        <table width="2000" border=0 cellpadding=0 cellspacing=0 class='borde_tab' id="ExporExcel"> 
        <? } else { ?>
...
//---------------------------------------------------------------
            ...
            if (strlen($_POST["s_RADI_NUME_RADI"])) {
                if ($sWhere != "")
                    $sWhere .= " and ";
                $HasParam = true;
                $sWhere = $sWhere . "$radi_nume_radi like " . tosql("%" . trim($_POST["s_RADI_NUME_RADI"]) . "%", "Text");
            }
            ...
            /**
             * Se recibe el nï¿½mero del expediente para bï¿½squeda
             * Fecha de modificaciï¿½n: 30-Junio-2006
             * Modificador: Supersolidaria
             */
            ...
            if (strlen($ps_SGD_EXP_SUBEXPEDIENTE) != 0) {
                 ...
                 * Fecha de modificaciï¿½n: 12-Septiembre-2006
                 * Modificador: Supersolidaria
                 */
            ...
            $eLen = strlen($_POST_["s_entrada"]);
            $ps_salida = strip(get_param("s_salida"));
            $sLen = strlen($ps_salida);
          
            if ($_POST["s_entrada"] != "9999") {
                if ($sWhere != "")
                    $sWhere .= " and ";
                $HasParam = true;
                $sWhere = $sWhere . "($radi_nume_radi like " . tosql("%" . trim($_POST["s_entrada"]), "Text") . ")";
            }
...
//-------------------------------
            ...
            /*             * ******************************************************************
              /**
             * Bï¿½squeda por parï¿½meto del expediente 
             ...
             */
            //Modificacion de la consulta para trabajar con la mejora de la busqueda por metadato - Grupo Iyunxi Ltda.
            if (strlen($ps_SGD_EXP_SUBEXPEDIENTE) != 0) {
            ...
            if($s_GUIA != '' )
             {  
                 $sSQL .= " INNER JOIN SGD_RENV_REGENVIO rvc ON rvc.RADI_NUME_GRUPO = R.RADI_NUME_RADI ";
                $sWhere .=" AND rvc.SGD_RENV_PLANILLA = '$s_GUIA' ";
             }
...
//-------------------------------
            ...
            /**
             * Bï¿½squeda por expediente
             * Fecha de modificaciï¿½n: 11-Agosto-2006
             * Modificador: Supersolidaria
             */
...
// Navigation begin
                ...
                if (($bEof && !$bEof->EOF) || $iPage != 1) {
                    ...
                    $sPages = "";
                    $iDisplayPages = 0;
                    $iNumberOfPages = 30; /* El nmero de pï¿½inas que aparecerï¿½ en el navegador al pie de la pï¿½ina */
    ...
    if ($rsaux->EOF || !$rsaux) {
        ...
        $ruta = "$ruta_raiz/" . $carpetaBodega . "tmp/Busqclasic" . date('Y_m_d_H_i_s') . ".xls";
        $f = fopen($ruta, 'w');
        
        if ($f) {
            rs2csvfile($rsaux, $f);
            echo "<a href='$ruta' target='_blank'><img style='border:0px' width='30' height='30' src='" . $ruta_raiz . "/imagenes/csv.png' alt='Archivo CSV'/>Archivo CSV</a>";
            echo "<br/><form action='expexcel.php' method='post' target='_blank' id='FormExport'>
                    <img style='border:0px' width='25' height='25' src='../imagenes/spreadsheet.png' class='botonExcel' />
                    <font size='3' color='blue'><u>Archivo XLS</u></font>
...
function buscar_prueba($nivelus, $tpRemDes, $whereFlds) {
    ...
    $ps_RADI_DEPE_ACTU = get_param("s_RADI_DEPE_ACTU");
    $ps_SGD_EXP_SUBEXPEDIENTE = trim($_POST['s_SGD_EXP_SUBEXPEDIENTE']);
