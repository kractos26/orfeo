...
class PintarFormulario {
   ...
   { 
       ...
       if($tipo == "select")
       {
          $this->control= $this->PintarListas($name, $tabla, $value, $option,$ayuda,$obligatorio,$evento,$funcionjs,$selectdefault);
       }
...
';
   ...
   function PintarListas($name, $tabla, $value, $option,$ayuda,$obligatorio,$evento,$funcionjs,$selectdefault){
        ...
        $lista= $resultado->GetMenu2( $name,$selectdefault,true, false, 0,'   title="'.$ayuda.'" class="form-control" id="'.$name.'" '.$fun.' '.$claseobligatorio.'' );
        $this->lista = $lista;
