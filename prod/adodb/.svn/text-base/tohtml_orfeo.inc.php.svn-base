...
GLOBAL $gSQLMaxRows,$gSQLBlockRows;
	...
	for ($i=0; $i < $ncols; $i++) {	
		...
		$hdr .= "<TH class=tituloListado>$fname</TH>";
	}
	$hdr .= "\n</tr>";
	if ($echo) print $hdr."\n\n";
	else $html = $hdr;
	...
	while (!$rs->EOF) {
	  if($ii==0) 
		...
		$s .= "<TR valign=top  class=textoOpcion>\n";
		for ($i=0; $i < $ncols; $i++) {
			...
			case 'D':
				if (!strpos($v,':')) {
					$s .= "	<TD class=$class_grid>".$rs->UserDate($v,"D d, M Y") ."&nbsp;</TD>\n";
					break;
				}
			case 'T':
				$s .= "	<TD  class=$class_grid>".$rs->UserTimeStamp($v,"D d, M Y, h:i:s") ."&nbsp;</TD>\n";
			break;
			case 'I':
			case 'N':
				$s .= "	<TD  class=$class_grid align=right>".stripslashes((trim($v))) ."&nbsp;</TD>\n";
			...
			*/
			...
			default:
				...
				$v = trim($v);
				if (strlen($v) == 0) $v = '&nbsp;';
				$s .= "	<TD class=$class_grid>". str_replace("\n",'<br>',stripslashes($v)) ."</TD>\n";
...
function arr2html(&$arr,$ztabhtml='',$zheaderarray='')
{
	...
	$s = "<TABLE $ztabhtml class=t_bordeGris>";//';print_r($arr);
