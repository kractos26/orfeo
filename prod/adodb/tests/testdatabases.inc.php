...
if (!empty($testmssql)) { // MS SQL Server via ODBC
	...
	$dsn = "PROVIDER=MSDASQL;Driver={SQL Server};Server=localhost;Database=northwind;uid=adodb;pwd=natsoft";
...
if (!empty($testmssql) && !empty($testado) ) { // ADO ACCESS MSSQL -- thru ODBC -- DSN-less
	...
	$myDSN="PROVIDER=MSDASQL;DRIVER={SQL Server};"
		. "SERVER=$server;DATABASE=NorthWind;UID=sa;PWD=natsoft;Trusted_Connection=No"  ;
