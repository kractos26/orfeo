...
{   if($_GET["ano"]&&$_GET["mes"]&&$_GET["dia"])$fenac = $_GET["ano"]."-".$_GET["mes"]."-".$_GET["dia"];
    ...
    $isql = "SELECT USUA_DOC, USUA_NOMB, DEPE_CODI, USUA_LOGIN, USUA_NACIM, USUA_AT, USUA_PISO, USUA_EXT,
			USUA_EMAIL, USUA_CODI FROM USUARIO WHERE USUARIO.USUA_LOGIN = '" .$_GET["usuLoginSel"] ."'";
    ...
    if($_GET["usuLoginSel"] <> $usuLogin)
    {
        $isql1 = $isql1."USUA_LOGIN = '".$usuLogin."', ";
		$isql = "INSERT INTO SGD_USH_USUHISTORICO (SGD_USH_ADMCOD, SGD_USH_ADMDEP, SGD_USH_ADMDOC, SGD_USH_USUCOD, SGD_USH_USUDEP, SGD_USH_USUDOC, SGD_USH_MODCOD, SGD_USH_FECHEVENTO, SGD_USH_USULOGIN) VALUES ($codusuario, $dependencia, '".$usua_doc."', ".$rs->fields["USUA_CODI"].", $dep_sel, '".$cedula."', 6, ".$sqlFechaHoy.", '".$usuLogin."')";
		$db->conn->Execute($isql);
    }
	...
	$isql1 = $isql1." USUA_NACIM =".$db->conn->DBTimeStamp($fenac).", ";
		$isql = "INSERT INTO SGD_USH_USUHISTORICO (SGD_USH_ADMCOD, SGD_USH_ADMDEP, SGD_USH_ADMDOC, SGD_USH_USUCOD, SGD_USH_USUDEP, SGD_USH_USUDOC, SGD_USH_MODCOD, SGD_USH_FECHEVENTO, SGD_USH_USULOGIN) VALUES ($codusuario, $dependencia, '".$usua_doc."', ".$rs->fields["USUA_CODI"].", $dep_sel, '".$cedula."', 53, ".$sqlFechaHoy.", '".$usuLogin."')";
		$db->conn->Execute($isql);
    }
...
<?
    ...
    if ($isql1 != "")
    {   $isql1 = "UPDATE USUARIO SET " .$isql1. " WHERE USUA_LOGIN = '".$_GET["usuLoginSel"]."'";
...
{	if($perfil=="Normal")
	...
	$isql = "INSERT INTO SGD_USH_USUHISTORICO (SGD_USH_ADMCOD, SGD_USH_ADMDEP, SGD_USH_ADMDOC, SGD_USH_USUCOD, SGD_USH_USUDEP, SGD_USH_USUDOC, SGD_USH_MODCOD, SGD_USH_FECHEVENTO,SGD_USH_USULOGIN) VALUES ($codusuario, $dependencia, '". $usua_doc."', ".$rs->fields["USUA_CODI"].", ".$dep_sel.", '".$cedula."', 1 , ".$sqlFechaHoy.", '".strtoupper($usuLogin)."')";
	$db->conn->Execute($isql);
