 ...
 *	Son tablas que tienen un codigo (digitado) y una descripci�n. P.E. : Tema, Resolucion.
...
{	$db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
	...
	{	$record = array();
		...
		$record_mtr['mrec_codi'] = $_POST['slc_cmb2'];
		$record_mtr['sgd_fenv_codigo'] = $_POST['txtId'];
		
		switch($_POST['btn_accion'])
		{	Case 'Agregar':
				{
					$tabla = 'SGD_FENV_FRMENVIO';
					$tabla_mtr ='sgd_mtr_mrecfenv';
					$sql = $db->conn->GetInsertSQL(&$tabla, $record, true, null);
					$insert_mtr = $db->conn->GetInsertSQL(&$tabla_mtr, $record_mtr, true, null);
					$ok = $db->conn->Execute($sql);
					$ok1 = $db->conn->Execute($insert_mtr);
					($ok && $ok1) ? $error = 3 : $error = 2;
				}break;
			Case 'Modificar':
				{	$ok = $db->conn->Replace('SGD_FENV_FRMENVIO', $record, 'SGD_FENV_CODIGO', true);
					$rs_m= $db->conn->Execute('select * FROM SGD_MTR_MRECFENV WHERE SGD_FENV_CODIGO='.$record['SGD_FENV_CODIGO']);
					if($rs_m){
					$okm = $db->conn->Execute('DELETE FROM SGD_MTR_MRECFENV WHERE SGD_FENV_CODIGO='.$record['SGD_FENV_CODIGO']);
					}
					if ($record_mtr['mrec_codi'])$ok1 = $db->conn->Replace('sgd_mtr_mrecfenv', $record_mtr, 'SGD_FENV_CODIGO', true);
					($ok) ? $error = 4 : $error = 2;
				}break;
			Case 'Eliminar':
	...
	$sql =	"SELECT s.SGD_FENV_DESCRIP, s.SGD_FENV_CODIGO, s.SGD_FENV_ESTADO, s.SGD_FENV_PLANILLA,m.mrec_codi
			FROM SGD_FENV_FRMENVIO s left join sgd_mtr_mrecfenv m on m.SGD_FENV_CODIGO=s.SGD_FENV_CODIGO  ORDER BY s.SGD_FENV_DESCRIP";
	...
	{	//Creamos el vector que contiene todas las Formas de envio con su respectiva informacion.
		...
		{	$v_fenv[$i]['nombre'] = trim($arr['SGD_FENV_DESCRIP']);
			...
			$arr['MREC_CODI']?$v_fenv[$i]['mrec'] = trim($arr['MREC_CODI']):$v_fenv[$i]['mrec']=0;
...
/*************implementacion de matriz relacion recepcion/envio***********/
$sql_mtr = "select mrec_desc ,mrec_codi from medio_recepcion order by 1 asc";	
$rs_med = $db->conn->Execute($sql_mtr);
if($rs_med && !$rs_med->EOF)
{
	$tmp1="0:&lt;&lt;SELECCIONE&gt;&gt;";
	$tmp2="Onchange=''";
	$cmb_mrec = $rs_med->GetMenu('slc_cmb2',false,$tmp1,false,false,"id='slc_cmb2' class='select' $tmp2");
}
?>
<html>
<head>
...
{	var strMensaje = "Por favor ingrese los datos.";
...
{	
	...
	{	document.getElementById('txtId').value = ve[i]['id'];
		...
		document.getElementById('slc_cmb2').value = ve[i]['mrec'];
	}
	else
	{	document.getElementById('txtId').value = '';
		...
		document.getElementById('slc_cmb2').value = '';
	}
}
</script>
...
<tr bordercolor = "#FFFFFF">
	...
	<td class="listado2">
		...
		<tr>
                    <td width="33%" align="center" valign="middle" colspan="2" class="titulos2">Relaci&oacute;n medios de recepci&oacute;n</td>
			
		</tr>
		<tr>
                    <td width="33%" colspan="2"><?=$cmb_mrec?></td>
