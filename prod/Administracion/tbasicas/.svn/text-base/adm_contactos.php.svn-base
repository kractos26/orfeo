...
<table width="75%" border="1" align="center" class=t_bordeGris>
	<tr class=timparr>
		...
		<td width="72%" class="listado2" height="1">
		    ...
		    <option value='1' <? ($_POST['idtipo'] == '1')? print "selected" : print ""; ?>>Empresas</option>
			</select>
		</td>
	</tr>
	<tr class=timparr>
    	...
    	if ($rs_idtipo)
    	{	echo $rs_idtipo->GetMenu2('ideps',$ideps_defa,"0:&lt;&lt; Seleccione La Empresa &gt;&gt;",false,0,"id='ideps' class='select' onchange=\"borra_datos();this.form.submit();\"");
	    	$rs_idtipo->Close();
    	}
    	else
    	{
    		echo("<select name='ideps' id='ideps' class='select'><option value=0>&lt;&lt;  Seleccione La Empresa&gt;&gt;</option></select>");
    	}
		?>
		</td>
	</tr>
	<tr>
		...
		<?	// Listamos las entidades/Empresas.
    	if ($rs_idctt)
    	{	echo $rs_idctt->GetMenu2('idctt',$idctt_defa,"0:&lt;&lt; Seleccione Contacto &gt;&gt;",false,0,"id='idctt' class='select' onchange=\"borra_datos();Actual();\"");
	    	$rs_idctt->Close();
    	}
    	else
    	{
    		echo("<select name='idctt' id='idctt' class='select'><option value=0>&lt;&lt; Seleccione Contacto &gt;&gt;</option></select>");
    	}
		?>
		</td>
	</tr>
