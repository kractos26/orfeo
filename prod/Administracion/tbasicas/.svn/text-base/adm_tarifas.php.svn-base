...
*
*	Permite la administracin de tarifas. La insercin y modificacin hace uso de la funcion
*	Replace de ADODB, la eliminacin est sujeta a que �sta NUNCA haya sido referenciada.
*/
...
{	$conn->SetFetchMode(ADODB_FETCH_ASSOC);
			...
			{	$conn->BeginTrans();
				...
				{	$record = array_slice($record,0,3,true);
					...
					{	$record['SGD_TAR_VALENV1'] = $_POST['txt_v1'];		//valor envio (local/grupo1)
						...
						$record['SGD_TAR_VALENV1G1'] = null;				//valor envio (grupo1)
						$record['SGD_TAR_VALENV2G2'] = null;				//valor envio (grupo2)
					}
					else
					{	$record['SGD_TAR_VALENV1'] = null;					//valor envio (local/grupo1)
						$record['SGD_TAR_VALENV2'] = null;					//valor envio (nacional/grupo2)
	...
	$Rs_fenv = $conn->Execute($sql_fenv); 	//Query en cache por 24 horas.
...
{	$msg_error = "<tr bordercolor='#FFFFFF'>
	...
	switch ($error)
	{
		...
		case 3:	$msg_error .= "Error al gestionar datos, comun&iacute;quese con el Administrador de sistema !!";break;	//ERROR EJECUCCI�N SQL
		case 4: $msg_error .= "<blink>Tarifa eliminada exitosamente</blink>";break;	// EXITO EN LA ELIMINACIN
		case 5: $msg_error .= "Error. No se puede eliminar registro. Existe hist&oacute;rico.";break;	// ERROR EN LA ELIMINACIN
	}
	$msg_error .= "</td></tr>";
}
?>
...
{	for(i=0;i<form.length;i++)
    ...
    if(parseInt(form.txt_lim1.value)>parseInt(form.txt_lim2.value))
    {
        alert('El limite inferior debe ser menor al superior' );
        return false;
    }
  	form.submit();
}
...
?>
	...
	<tr bordercolor="#FFFFFF">
		<td width="16%" align="center" class="listado2"><input name="txt_lim1" id="txt_lim1" type="text" size="6" maxlength="6"></td>
		<td align="center" class="listado2"><input name="txt_lim2" id="txt_lim2" type="text" size="6" maxlength="6"></td>
