...
if ($db) {
            ...
            case 'Modificar': { /* Las reglas del negocio para la inactivacion de una dependencia son:
                 ...
                 */
                        ...
                        if (isset($_POST['slc_tr' . $tmp1])) {
                            ...
                            switch ($db->driver) {
                                ...
                                case 'mssql':
                                    ...
                                    $sql_rtp = "select * from radicado where " . $db->conn->substr . '(convert(char(14),radi_nume_radi),14,1)=' . $tmp1 . " and radi_depe_radi=" . $_POST['txtIdDep'];
                                    break;
                                case 'postgres':
                                default:
                                    ...
                                    $sql_rtp = "select * from radicado r where " . $db->conn->substr . '(' . $radi_nume_radi . ',-1)=\'' . $tmp1 . "' and radi_depe_radi=" . $_POST['txtIdDep'];
                                    break;
                            }
                            $ADODB_COUNTRECS = true;
                            ...
                            if ($vlr_nxt < $vlr_act) {
                                ...
                                $valorTP = $_POST['slc_tr' . $tmp1] ? $_POST['slc_tr' . $tmp1] : 'null';
                                $record['DEPE_RAD_TP' . $tmp1] = $valorTP;
                            }
                        }
                    }
                    ...
                    if ($oka && $okb && $okc) {
                        ...
                        $ok3 = true;
                        if ($_POST['Slc_VoBo'] > 0) {
                            $sqlVoBoMod = " UPDATE usuario SET usua_vobo_perm = 0 where usua_vobo_perm=1 and depe_codi=" . $_POST['id'];
                            $rsVoBo = $db->conn->Execute($sqlVoBoMod);
                            $sqlVoBoMod = " UPDATE usuario SET usua_vobo_perm = 1 where usua_doc='" . $_POST['Slc_VoBo'] . "' and depe_codi=" . $_POST['id'];
                            $rsVoBo = $db->conn->Execute($sqlVoBoMod);
                        }
    ...
    if ($rs) {
        ...
        if ($_POST['id'] > 0) {
            ...
            $sqlVoBo = "select usua_login, usua_nomb,usua_doc, usua_vobo_perm from usuario where usua_vobo_perm=1 and depe_codi=" . $_POST['id'];
            $sqlBoss = "select usua_login, usua_nomb,usua_doc, usua_vobo_perm from usuario where usua_codi=1 and depe_codi=" . $_POST['id'];
            $rsVoBo = $db->conn->Execute($sqlVoBo);
            $rsBoss = $db->conn->Execute($sqlBoss);
            if ($rsVoBo && !$rsVoBo->EOF) {
                $UsrVoBo = " " . $rsVoBo->fields["USUA_NOMB"] . " ";
            } else {
                $sqlVoBo2 = "select usua_login, usua_nomb, usua_doc, usua_vobo_perm from usuario where usua_codi=1 and depe_codi=" . $_POST['id'];
                $rsVoBo2 = $db->conn->Execute($sqlVoBo2);
                if ($rsVoBo2 && !$rsVoBo2->EOF) {
                    $UsrVoBo = " Sin parametrizar "; //$rsVoBo2->fields["USUA_NOMB"]
                } else {
                    $UsrVoBo = "  Dependencia sin Usuarios  ";
                }
            }
            if ($rsBoss && !$rsBoss->EOF) {
                $UsrBoss = " " . $rsBoss->fields["USUA_NOMB"] . " ";
            } else {
                $UsrBoss = "  Dependencia sin Usuarios  ";
            }
            $sqlUsrDep = "select usua_nomb,usua_doc from usuario where depe_codi=" . $_POST['id'];
            $rsUsrDep = $db->conn->Execute($sqlUsrDep);
            $UsrVoBo .= $rsUsrDep->GetMenu2('Slc_VoBo', 'USUA_NOMB', '0:&lt;&lt seleccione &gt;&gt;', false, false, 'Class="select" id="Slc_VoBo"');
    ...
    $arrNivel = array('0' => "P&uacute;blico", '1' => "Privado", '2' => "Dependencia");
...
switch ($error) {
    ...
    case 9: // Error en la modificacion de la dependencia
        $error_msg .= "El consecutivo para el tipo de radicado $tpr de la nueva dependencia (" . $_POST['slc_tr' . $tmp1] . ")($vlr_nxt) es menor que el actual (" . $depObj->getSecRadicTipDepe($_POST['txtIdDep'], $tmp1) . ")($vlr_act).";
        break;
    case 10:
        ...
        <script language="Javascript">
            ...
            {	var pos=false;
                if (x == '')
                {
                    ...
                    pedirCombosDivipola('<?php echo $ruta_raiz ?>/cCombos.php', 'NA','todos',1,170,11,1);
                    act_pes2('');
...
?>
    ...
    {	if (id_mcpio == 0) return;
        ...
        pedirCombosDivipola('<?php echo $ruta_raiz ?>/cCombos.php', 'NA','todos',str[0]*1,str[1]*1,str[2]*1,str[3]*1);
    }
    ...
    function pedirCombosDivipola(fuenteDatos, divID,tipo,continente,pais,departamento,municipio)
    {
        if(xmlHttp)
        {
            ...
            try
            {
                xmlHttp.open("GET", fuenteDatos+"?tipo="+tipo+"&continente="+continente+"&pais="+pais+"&departamento="+departamento+"&municipio="+municipio+"&ruta_fuente=<?php echo $ruta_raiz ?>");
    ...
    <body onload="pedirCombosDivipola('<?php echo $ruta_raiz ?>/cCombos.php', 'NA','todos',<?= $cont ?>,<?= $pais ?>,<?= $dpto ?>,<?= $muni ?>);">
            ...
            <div class="tabber" id="tab1">
                <div class="tabbertab" title="B&aacute;sicos">
                    <table width="100%" border="1" align="center" class="t_bordeGris">
                            ...
                            <td colspan="5" class="listado2">
                                <table>
                                    <tr class=e_tablas>
                                        ...
                                        <td  align="center" class="listado2"><?php echo $continente; ?><font color="Red" >*</font></td>
                                        <td  align="center" class="listado2">Pais</td>
                                        <td  align="center" class="listado2"><div id="DivPais"><select class="select" id="selPais"><option value="0">&lt;&lt Seleccione &gt;&gt</select><font color="Red" >*</font></div></td>
                                    </tr>
                                    <tr class=e_tablas>
                                        <td  align="center" class="listado2">Departamento</td>
                                        <td  align="center" class="listado2"><div id="DivDepto"><select class="select" id="selDepto"><option value="0">&lt;&lt Seleccione &gt;&gt</select><font color="Red" >*</font></div></td>
                                        <td  align="center" class="listado2">Municipio</td>
                                        <td  align="center" class="listado2"><div id="DivMnpio"><select class="select" id="selMnpio"><option value="0">&lt;&lt Seleccione &gt;&gt</select><font color="Red" >*</font></div></td>
                                    </tr>
                                </table>
                            </td>
                        </tr>
                        ...
                        <? if ($_POST['id'] > 0) { ?>
                            <tr>
                                <td align="left" class="titulos2"><b>&nbsp;Usuario Vo.Bo</b></td>
                                <td colspan="5" class="listado2">
                                    <?php
                                    echo $UsrVoBo;
                                    ?>
                            ...
                            <tr>
                                <td align="left" class="titulos2"><b>&nbsp;Usuario Jefe</b></td>
                                <td colspan="5" class="listado2">
                                    <?php
                                    echo $UsrBoss;
                                    ?>
