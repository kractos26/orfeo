...
{	
	...
	{	
		...
		$sqlw.= "AND r.radi_depe_radi $dep AND r.radi_usua_actu !=0 AND ".$radEntrada;
		...
		if($permitir_busq > 0)
		{
		  ...
		  {    
			while(!$rs->EOF)
			{         
                                    $sql="SELECT COUNT(CONSECUTIVO)+1 AS NUM FROM CONSECUTIVOPLANILLA";

                                    $rsnpla= $conn->Execute($sql);
            ...
            $his->insertarHistorico($resultado, $_SESSION["dependencia"], $_SESSION["codusuario"], $_SESSION["dependencia"],
                    $_SESSION["codusuario"],$msg2, 69);
