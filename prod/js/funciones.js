...
// Función para no sobrepara la longitud máxima definida para un campo tipo textarea.
var contenidoTextarea = "";
function validaLongitud( numCaracteresPermitidos, campoValidar ){
	numCaracteres = campoValidar.value.length;

	if( numCaracteres > numCaracteresPermitidos ){
		campoValidar.value = contenidoTextarea;
	}else{
		contenidoTextarea = campoValidar.value;
	}
}